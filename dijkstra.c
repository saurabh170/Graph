...
void shortest_path(struct node * graph[], int n, int a, int b)
{
	dijkstra(graph,n,a);
...
void shortest_pathb(struct node * graph[], int n, int a, int b)
{
	dijkstrab(graph,n,a);
	struct node * v = (struct node*)malloc(sizeof(struct node));
	struct node * temp = (struct node*)malloc(sizeof(struct node));
...
int main()
{
	...
	graph[4]->value=5;
	graph[5]->value=6;
	add_edge(1,2,3,n,graph);
	add_edge(2,4,4,n,graph);
	add_edge(4,6,7,n,graph);
	add_edge(5,6,5,n,graph);
	add_edge(3,5,2,n,graph);
	add_edge(1,3,1,n,graph);
	add_edge(2,5,10,n,graph);
	add_edge(1,6,12,n,graph);
	shortest_path(graph,n,1,6);

	shortest_pathb(graph,n,2,6);
	return 0;
}